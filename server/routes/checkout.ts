...
const getUserIdFromHeaders = (req: Request): string => {
...
router.get('/user/:userIdentifier', async (req: Request, res: Response): Promise<void> => {
  try {
...
router.put('/:orderId/payment', async (req: Request, res: Response): Promise<void> => {
  try {
    const { orderId } = req.params;
  ...
  try {
    ...
    const stats = {
      ...
      byPaymentMethod: Object.fromEntries(
        byPaymentMethod.map((m) => [m._id, { count: m.count, total: m.total }])
      ),
    };
