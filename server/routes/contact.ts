...
router.get('/messages', async (req: Request<any, any, any, ContactQuery>, res: Response): Promise<void> => {
  try {
    ...
    res.json({
      ...
      pagination: {
        ...
        limit: parseInt(limit as string),
        skip: parseInt(skip as string),
        remaining: total - (parseInt(skip as string) + parseInt(limit as string))
      }
    });
  } catch (error) {
