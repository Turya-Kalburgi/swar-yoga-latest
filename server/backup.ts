 ...
 */
async function initBackupsDir(): Promise<void> {
  try {
 ...
 */
async function getAllBackups(): Promise<string[]> {
  try {
 ...
 */
async function createDailyBackup(): Promise<BackupResult> {
  try {
 ...
 */
async function restoreFromBackup(backupFilename: string): Promise<RestoreResult> {
  try {
    const backupPath = path.join(BACKUPS_DIR, backupFilename);
