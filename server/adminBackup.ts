 ...
 */
export async function createSignoutBackup(adminId: string, email: string): Promise<BackupResult> {
  try {
 ...
 */
export async function createManualBackup(adminId: string, email: string): Promise<BackupResult> {
  try {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
 ...
 */
export async function getBackupStats(): Promise<{ total: number; signoutCount: number; manualCount: number }> {
  try {
 ...
 */
export async function getBackupFile(backupName: string): Promise<Buffer> {
  try {
    const backupPath = path.join(BACKUPS_DIR, backupName);
    await fs.access(backupPath);
